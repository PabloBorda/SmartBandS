...
public class SellActivity extends Activity {
	...
	private boolean publish_product() {
		...
		if (validate_form()) {
			...
			p.name = product_title.getText().toString();
			p.price = price_edit.getText().toString();
			p.description = description_edit.getText().toString();
			p.tags = new ArrayList<String>(Arrays.asList(tags_edit.getText()
					.toString().split(" ")));
			p.deliver = deliver_checkbox.toString();
			p.delivery_ratio = ratio_edit.getText().toString();
			p.pics = pics;
			Store s = new Store();
			s.getProducts().add(p);
			...
			params.setParameter("name", p.name);
			params.setParameter("price", p.price);
			params.setParameter("description", p.description);
			params.setParameter("time", new Date());
			try {
				String jout = mapper.writeValueAsString(s);
